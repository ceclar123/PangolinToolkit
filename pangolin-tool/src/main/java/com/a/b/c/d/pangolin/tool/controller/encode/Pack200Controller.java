...
import com.a.b.c.d.pangolin.util.encode.Pack200Util;
...
public class Pack200Controller implements Initializable {
    @FXML
    ...
    public void btnEncodeOnAction(ActionEvent event) {
        ...
        try {
            this.txtTo.setText(new String(Base64.getEncoder().encode(Pack200Util.compress(input.getBytes(charset))), charset));
        } catch (Exception e) {
    ...
    public void btnDecodeOnAction(ActionEvent event) {
        ...
        String charset = this.getCharsetName();
        try {
            this.txtFrom.setText(new String(Pack200Util.decompress(Base64.getDecoder().decode(input.getBytes(charset))), charset));
        } catch (Exception e) {
            AlertUtil.showAlert(Alert.AlertType.ERROR, "错误", e.getMessage(), ExceptionUtil.getStackTrace(e));
        }
    }
