...
import com.a.b.c.d.pangolin.util.encode.SnappyUtil;
...
public class SnappyController implements Initializable {
    @FXML
    ...
    public void btnDecodeOnAction(ActionEvent event) {
        ...
        String charset = this.getCharsetName();
        try {
            this.txtFrom.setText(new String(SnappyUtil.decompress(Base64.getDecoder().decode(input.getBytes(charset))), charset));
        } catch (Exception e) {
            AlertUtil.showAlert(Alert.AlertType.ERROR, "错误", e.getMessage(), ExceptionUtil.getStackTrace(e));
        }
    }
