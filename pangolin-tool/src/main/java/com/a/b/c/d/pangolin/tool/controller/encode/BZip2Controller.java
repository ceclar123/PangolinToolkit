...
import com.a.b.c.d.pangolin.util.encode.BZip2Util;
...
public class BZip2Controller implements Initializable {
    @FXML
    ...
    public void btnEncodeOnAction(ActionEvent event) {
        ...
        try {
            this.txtTo.setText(new String(Base64.getEncoder().encode(BZip2Util.compress(input.getBytes(charset))), charset));
        } catch (Exception e) {
    ...
    public void btnDecodeOnAction(ActionEvent event) {
        ...
        String charset = this.getCharsetName();
        try {
            this.txtFrom.setText(new String(BZip2Util.decompress(Base64.getDecoder().decode(input.getBytes(charset))), charset));
        } catch (Exception e) {
            AlertUtil.showAlert(Alert.AlertType.ERROR, "错误", e.getMessage(), ExceptionUtil.getStackTrace(e));
        }
    }
